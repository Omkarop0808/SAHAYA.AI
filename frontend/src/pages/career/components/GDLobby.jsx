...
function GDLobbyInternal({ config, eduData, onEndGd }) {
  ...
  if (matchData && !roomStarted) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-white min-h-[60vh]">
        ...
        <p className="text-white/70 mb-8 max-w-sm text-center">
          Your Group Discussion room is ready. Please confirm your attendance.
        </p>

        <div className="bg-white/[0.05] border border-white/10 rounded-2xl p-8 max-w-sm w-full text-center flex flex-col items-center shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
          <div className="text-5xl font-mono font-bold mb-2 text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]">
            {readyCount}{' '}
            <span className="text-2xl text-white/40">/ {matchData.participants.length}</span>
          </div>
          ...
          {!isReady ? (
            <button
              ...
              className="w-full py-4 rounded-xl font-bold text-lg bg-[linear-gradient(135deg,#10B981,#059669)] text-white shadow-xl hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:brightness-110 active:scale-95 transition-all transform hover:-translate-y-1"
            >
              I'm Ready
            </button>
          ) : (
            <div className="w-full py-4 rounded-xl font-bold text-lg bg-white/5 border border-white/10 text-white/50 cursor-not-allowed flex items-center justify-center gap-3">
        ...
        <p className="text-xs text-white/40 font-medium mt-6 text-center max-w-xs">
  ...
  return (
    <div className="flex flex-col items-center justify-center p-12 text-white min-h-[60vh]">
        ...
        <div className="flex flex-col gap-3">
          {matchingQueue.length === 0 ? (
            ...
            matchingQueue.map((user, idx) => (
              <div
                key={idx}
                className="flex items-center gap-3 bg-white/[0.04] rounded-lg p-3 border border-white/[0.05]"
              >
          ...
          {Array.from({ length: Math.max(0, config.gdParticipants - matchingQueue.length) }).map(
            (_, idx) => (
              <div
                key={`empty-${idx}`}
                className="flex items-center gap-3 bg-white/[0.01] border border-dashed border-white/10 rounded-lg p-3 opacity-50"
              >
          ...
          <div className="mt-6 pt-4 border-t border-white/10">
            ...
            <div className="flex flex-col gap-2">
              {otherQueue.map((user, idx) => (
                <div
                  key={`other-${idx}`}
                  className="flex flex-col bg-white/[0.02] border border-white/[0.05] rounded p-2 px-3"
                >
                  <div className="text-sm text-white/70 font-medium">{user.name || 'Student'}</div>
      ...
      <div className="mt-12 px-6 py-3 bg-gray-800 rounded-lg text-sm text-gray-300 shadow-xl border border-gray-700">
        Socket Connected:{' '}
