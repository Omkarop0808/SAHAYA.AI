export default function PhaseCard({ phase, onToggleItem, expanded, onToggleExpand }) {
  const itemRow = (item, kind) => (
    <label key={item.id} className="flex items-start gap-2 text-sm text-[var(--career-text)] text-opacity-80">
      <input
        type="checkbox"
    ...
    <div className="career-card">
      <button type="button" className="w-full flex items-center justify-between text-left" onClick={onToggleExpand}>
        <div>
          ...
          <p className="text-xs text-[var(--career-muted)] mt-1 uppercase tracking-wider">State: {phase.state?.replace('_', ' ')}</p>
        </div>
      ...
      {expanded && (
        <div className="mt-4 space-y-4">
          <div>
            <div className="text-xs text-[var(--career-muted)] uppercase tracking-[0.2em] mb-2">Skills</div>
            <div className="space-y-2">{(phase.skills || []).map((item) => itemRow(item, 'skills'))}</div>
          </div>
          <div>
            <div className="text-xs text-[var(--career-muted)] uppercase tracking-[0.2em] mb-2">Resources</div>
            <div className="space-y-2">{(phase.resources || []).map((item) => itemRow(item, 'resources'))}</div>
          </div>
          <div>
            <div className="text-xs text-[var(--career-muted)] uppercase tracking-[0.2em] mb-2">Projects</div>
            <div className="space-y-2">{(phase.projects || []).map((item) => itemRow(item, 'projects'))}</div>
          </div>
          <div className="rounded-[10px] border border-[var(--career-border)] bg-[var(--career-surface)] p-3 text-sm text-[var(--career-text)] text-opacity-80">
