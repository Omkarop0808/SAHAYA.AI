...
export default function DailyQuestsPanel({ world = 'study' }) {
  ...
  if (loading) {
    if (isCareer) return <div className="animate-pulse bg-[var(--career-surface)] border border-[var(--career-border)] rounded-2xl h-40"></div>;
    ...
    <>
      ...
      <div className={isCareer ? "career-card p-5" : "bg-white border-2 border-[#0D0D0D] rounded-[16px] p-5 shadow-[4px_4px_0_#E0E0E0]"}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className={isCareer ? "font-display font-extrabold text-xl text-[var(--career-text)]" : "font-display font-extrabold text-xl text-[#0D0D0D]"}>Daily Quests</h3>
            <p className={isCareer ? "text-sm text-[var(--career-muted)]" : "text-sm text-[#555555]"}>Earn XP and rank up!</p>
          </div>
        ...
        <div className="space-y-3">
          {quests.map(q => {
            ...
            return (
              <div key={q.id} className={`quest-card flex items-center justify-between gap-4 p-3 rounded-xl relative overflow-hidden ${isCareer ? "border border-[var(--career-border)] bg-[var(--career-surface)]" : "border-2 border-[#E0E0E0] bg-white"}`}>
                <div className="flex-1 min-w-0 z-10 relative">
                  <div className="flex justify-between items-end mb-1">
                    <h4 className={`font-bold text-sm truncate ${isCareer ? "text-[var(--career-text)]" : "text-[#0D0D0D]"}`}>{q.title}</h4>
                    <span className={`text-xs font-bold ${isCareer ? "text-[var(--career-accent)]" : "text-[#0D0D0D]"}`}>+{q.xp_reward} XP</span>
                  </div>
                  <p className={`text-xs truncate mb-2 ${isCareer ? "text-[var(--career-muted)]" : "text-[#555555]"}`}>{q.description}</p>
                  <div className={`h-2 w-full rounded-full overflow-hidden ${isCareer ? "bg-black/[0.05]" : "bg-[#F0F0F0] border border-[#E0E0E0]"}`}>
                     <div className={`h-full transition-all ${isCareer ? "bg-[var(--career-accent)]" : "bg-[#FFB6C1]"}`} style={{ width: `${progressPercent}%` }} />
                  </div>
                  <div className={`text-[10px] text-right mt-1 font-bold ${isCareer ? "text-[var(--career-muted)]" : "text-[#0D0D0D]"}`}>{q.progress}/{q.target}</div>
                </div>
                
                <div className="z-10 relative flex-shrink-0 ml-2">
                  {q.status === 'pending' && <div className={`text-xs font-bold px-3 py-1 cursor-not-allowed ${isCareer ? "text-[var(--career-muted)] opacity-50" : "text-[#999999]"}`}>Locked</div>}
