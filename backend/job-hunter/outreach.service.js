...
const STATUS_FLOW = ['drafted', 'sent', 'replied', 'converted'];

export async function updateOutreachStatus(userId, outreachId, nextStatus) {
  const target = String(nextStatus || '').toLowerCase();
  if (!STATUS_FLOW.includes(target)) throw new Error('Invalid outreach status.');
  const current = await findOne(COL_OUTREACH, (x) => x.userId === userId && x.id === outreachId);
  if (!current) return null;
  const currentIdx = STATUS_FLOW.indexOf(String(current.status || 'drafted').toLowerCase());
  const nextIdx = STATUS_FLOW.indexOf(target);
  if (nextIdx < currentIdx || nextIdx - currentIdx > 1) {
    throw new Error('Invalid status transition. Move one step at a time.');
  }
  const updated = await updateOne(
    COL_OUTREACH,
    (x) => x.userId === userId && x.id === outreachId,
    { status: target, updatedAt: nowIso() },
  );
  return updated;
}
