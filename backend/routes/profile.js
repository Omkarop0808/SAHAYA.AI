...
router.get('/edu', authMiddleware, async (req, res) => {
  const data = await findOne('edu_data', d => d.userId === req.userId);
...
router.post('/edu', authMiddleware, async (req, res) => {
  ...
  await upsertOne('edu_data', d => d.userId === req.userId, doc);

  // Mark user as having completed data collection
  await updateOne('users', u => u.id === req.userId, { hasCompletedDataCollection: true });
